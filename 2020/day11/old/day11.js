...
let finalInput = `LLLLLLLL.LLLL.LLLLL.LLLLLLLLLLLLLL.LLLLLLLLLLLLLL.LLLLL.LLLLLLLLLLLLLL.LLLLLLL.LLLLLL.LLLLLLLLLL.L
...
LLLLLLLL.LLLLLL.LLL.LLLLLLLLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLL`.split('\n').map(el => el.split(''))
// console.log(rawInput)

let rawInput = sample
// let rawInput = finalInput
console.log(`+--------------------+\n| L -> empty seat    |\n| . -> floor         |\n| # -> occupied seat |\n+--------------------+`)

const inBetween = (value, min, max) => value >= min && value <= max
let hasChanged = true

const findImmediatelyAdjacentSeats = (_input, {x, y}) => {
  let adjacentSeats = []
  for(let i = x - 1; i <= x + 1; i++) {
    for(let j = y - 1; j <= y + 1; j++) {
      if(inBetween(i, 0, _input.length - 1)) {
        const row = _input[i]
        if(inBetween(j, 0, row.length - 1)) {
          if(i !== x || j !== y) {
            adjacentSeats.push(_input[i][j])
          }
        }
      }
    }
...
const findFirstSeatInEachDirection = (_input, {x, y}) => {
  let visibleSeats = []
  
  const travelThroughArray = (array, startX, startY, stepX, stepY) => {
    for(let i = startX; i < array.length; i = i + stepX) {
      for(let j = startY; j < array[i].length; j = j + stepY) {
        if(i !== startX && j !== startY && (array[i][j] === 'L' || array[i][j] === '#')) {
  ...
  // right -> 1,0
  travelThroughArray(_input, x, y, 1, 0)
...
function goThroughSeats(input) {
  iterations++
  let newInput = _.cloneDeep(input)
  for(let i = 0; i < input.length; i++) {
    for(let j = 0; j < input[i].length; j++) {
      const thisSpot = input[i][j]
      const adjacentSpots = findFirstSeatInEachDirection(input, {x: i, y: j})
      if(adjacentSpots.length !== 0) {
        const howManySeatsTaken = adjacentSpots.filter(el => el.input === '#').length
        console.log(howManySeatsTaken, {i, j})
        if (thisSpot === 'L' && howManySeatsTaken === 0) {
          newInput[i][j] = '#'
        }
        if (thisSpot === '#' && howManySeatsTaken >= 5) {
          newInput[i][j] = 'L'
        }
      }
    }
...
let seatAtSpecificMoment = _.cloneDeep(rawInput)

function part1() {  
  while(hasChanged) {
    const iteratedSeat = goThroughSeats(seatAtSpecificMoment)
    if(_.isEqual(iteratedSeat, seatAtSpecificMoment)) {
      hasChanged = false
    } else {
      seatAtSpecificMoment = _.cloneDeep(iteratedSeat)
    }
  }
